...
public class SqlFormatter {
    ...
    public static String format(String source, boolean fixHql) {
        if (fixHql && isHql(source)) {
            source = "select * " + StringKit.trimLeft(source);
        }
        return new FormatProcess(source).perform().trim();
    }

    private static boolean isHql(String source) {
        source = StringKit.trimLeft(source);
        return (StringKit.startsWith(source, "from", true));
    }
